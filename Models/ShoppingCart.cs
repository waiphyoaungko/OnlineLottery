...
namespace OnlineLottery.Models
{
    public class ShoppingCart
    {
        ...
        public void AddToCart(Ticket ticket, int amount)
        {
            var shoppingCartItem = _appDbContext.ShoppingCartItems.SingleOrDefault(
                s => s.Ticket.TicketId == ticket.TicketId && s.ShoppingCartId == ShoppingCartId);
        ...
        public List<ShoppingCartItem> GetShoppingCartItems()
        {
            return ShoppingCartItems ?? (ShoppingCartItems = _appDbContext.ShoppingCartItems.Where(c => c.ShoppingCartId == ShoppingCartId)
                .Include(s => s.Ticket).Include(x=>x.Ticket.SubCategories)
                .ToList());
        }
        ...
        public decimal GetShoppingCartTotal()
        {
            var total = _appDbContext.ShoppingCartItems.Where(c => c.ShoppingCartId == ShoppingCartId)
                .Select(c => c.Ticket.Price * c.Amount).Sum();
